...
export default function BurnoutDashboardPage() {
  ...
  const createBurnoutSession = async () => {
    ...
    try {
      ...
      const { data: survey, error: surveyError } = await (supabase as any)
        ...
        .insert({
          user_id: user.id,
          title: `Burnout Assessment - ${new Date().toLocaleDateString('th-TH')}`,
  ...
  return (
    <div className="px-2 sm:px-0">
      <div className="mb-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">แบบประเมิน Burnout</h1>
        <p className="text-sm sm:text-base text-gray-600 mt-1">
          ประเมินระดับ Burnout ของทีมด้วย 3 คำถาม พร้อมผลลัพธ์แบบ Realtime
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-md p-6 mb-8 border-2 border-orange-200">
        ...
        >
          {creating ? 'กำลังสร้าง...' : 'เริ่มแบบประเมิน Burnout 🚀'}
        </button>
      </div>
